...
> **重要**: 为避免模型生成重复文字、多余肢体、变形结构等问题，每个图像生成请求必须包含负面提示词。

**常见问题与负面提示词**：

| 问题类型 | 负面提示词 |
|---------|-----------|
| 重复文字 | 不要重复文字、文字不重叠、文字清晰单次出现 |
| 多余肢体 | 不要多余的手、不要第三只手、两只手、四肢完整 |
| 变形结构 | 人体结构正确、比例协调、五官端正 |
| 画面模糊 | 画面清晰、线条分明、无模糊 |
| 风格混乱 | 风格统一、画风一致 |
...
```
负面提示词（追加到 prompt 末尾）：
不要重复文字，不要多余的手，人体结构正确，两只手，四肢完整，比例协调，画面清晰
```
...
```
{panel_image_prompt}，{style_seed}，高质量，连环画，画面清晰，人体结构正确，不要重复文字，不要多余的手
```

**提示词长度建议**: 单 Panel 以 80-150 中文字符为宜（不含负面提示词）
...
| 检查项 | 通过标准 | 不通过则重新生成 |
|-------|---------|----------------|
| **文字重复** | 文字清晰，无重复 | 发现重复文字 |
| **手部数量** | 人物双手数量正确 | 多余或缺失的手 |
| **人体结构** | 比例协调，结构正确 | 变形、扭曲 |
| **五官位置** | 眼睛、鼻子、嘴巴位置正常 | 歪斜、错位 |
| **画面清晰度** | 线条清晰，无模糊 | 模糊不清 |
