...
async function main() {
  ...
  if (!AUTO_MODE) {
    console.log('\n========================================');
    console.log('请输入模型平台地址');
    console.log('========================================');
    console.log('示例: https://your-platform-url.com/chat');
    config.targetUrl = await question('\n请输入平台地址: ');
    config.targetUrl = config.targetUrl.trim();
    if (!config.targetUrl) {
    ...
    // 自动模式下检查是否通过环境变量传入
    config.targetUrl = process.env.PLATFORM_URL || '';
    if (!config.targetUrl) {
      console.error('错误: 自动模式下需要通过环境变量 PLATFORM_URL 指定平台地址');
      process.exit(1);
    }
  }

  console.log(`\n平台地址: ${config.targetUrl}\n`);
  ...
  // 检查是否需要登录（检测URL变化或登录页面特征）
  const currentUrl = page.url();
  if (currentUrl.includes('login') || currentUrl !== config.targetUrl.split('?')[0]) {
    if (AUTO_MODE) {
      console.log('\n检测到需要登录，自动等待30秒...');
