...
检查范围：**标题、正文内容、评论、标签**——任一字段包含以下关键词之一即视为相关：

| 关键词 | 说明 |
|--------|------|
| `ERNIE-Image` | 标准写法（含连字符） |
| `ERNIE-Image-Turbo` | Turbo 变体（含连字符，`ERNIE-Image` 的子集） |
| `ERNIE_Image` | 下划线写法 |

匹配不区分大小写；`ERNIE-Image` 同时覆盖 `ERNIE-Image-Turbo`。
四个字段均不含上述关键词的条目一律过滤，不进入报告。
...
相关性匹配词（标题/正文/评论/标签需包含其一，不区分大小写）：`ERNIE-Image` · `ERNIE-Image-Turbo` · `ERNIE_Image`
